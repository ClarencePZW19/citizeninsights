...
def generate_visualisation(document, dataset_id):
    init_vertexai()
    responses_vis = model.generate_content(
        ["""
...
Choropleth Map: Displays variations in data across geographic areas using color shading.
Point Map: Uses points to represent data locations on a map, often with varying sizes or colors.
...
Ensure to pass page a json that can be mapped to the parameters of the function. 
The json should declare which visualization it is calling.

E.g. { viz_type: heatmap, data:...} where the … is the data that has been cleaned and processed for use in the visulization.
Return ONLY the json with the viz_type: and the processed data:, other parameters relevant to the function and nothing else. Do not add any commentary after. Only ever give back the formatted JSON and only the JSON. This format MUST be enforced as the json will be used to parse data to another function.
